...
void STServo::begin(HardwareSerial& serial, uint32_t baud, int8_t rxPin, int8_t txPin) {
  _serial = &serial;
...
void STServo::sendPacket(uint8_t id, uint8_t instr, const uint8_t* params, uint8_t paramLen) {
...
void STServo::writeRegisters(uint8_t id, uint8_t addr, const uint8_t* data, uint8_t len) {
  uint8_t params[12];
  params[0] = addr;
...
bool STServo::readRegisters(uint8_t id, uint8_t addr, uint8_t len, uint8_t* out) {
...
bool STServo::writePosition(uint8_t id, uint16_t position, uint16_t speed, uint8_t acc) {
  ...
  uint8_t data[7] = {
      acc, static_cast<uint8_t>(position & 0xFF), static_cast<uint8_t>((position >> 8) & 0xFF), 0,
      0,   static_cast<uint8_t>(speed & 0xFF),    static_cast<uint8_t>((speed >> 8) & 0xFF),
  };
...
bool STServo::readPosition(uint8_t id, uint16_t& position) {
  ...
  position = static_cast<uint16_t>(buf[0]) | (static_cast<uint16_t>(buf[1]) << 8);
  return true;
}
...
bool STServo::readSpeed(uint8_t id, int16_t& speed) {
  ...
  uint16_t raw = static_cast<uint16_t>(buf[0]) | (static_cast<uint16_t>(buf[1]) << 8);
  // Sign-magnitude: bit 15 is the direction sign.
  speed = (raw & 0x8000) ? -static_cast<int16_t>(raw & 0x7FFF) : static_cast<int16_t>(raw);
  return true;
}
